...
- **Foundations resource explainer (`GET /foundations/week/:n/explain`).** A
  ZS-generated 60-second framing of why this week's curated resource was chosen
  and what to look for while reading or watching it. Cached per week in GCS via
  the existing deepdive pattern (one entry keyed by a hash of week, theme and
  primary resource URL, identical for every student on that week, regenerated
  only when the curated URL changes). System prompt enforces curation framing,
  no placement promises, no in-model arithmetic, Hinglish welcomed. Refused on
  the gate weeks (4, 8, 12). Static fallback rendered when ZS is unavailable.
- **Foundations reflection prompts (`GET /foundations/week/:n/reflect`).** Three
  retrieval-style reflection questions per week in the Bjork desirable-difficulty
  tradition, grounded in the shipped artifact, generated by ZS once per week and
  cached in GCS like the explainer. Rendered as an ordered list at the bottom of
  every coach-on week page so the learner closes the tab on recall, not
  recognition. Refused on the gate weeks. Static three-prompt fallback when ZS
  is unavailable.
- **Foundations artifact review (`POST /foundations/review`).** A pasted README
  (8 KB hard cap) goes to ZS and the coach replies with exactly one thing the
  README does well, one thing the week brief asked for that is missing, and one
  nudging question that pushes the learner to verify their own work. Never
  grades, never assigns a score, never writes code, never rewrites the README.
  Treats the README text as data only, not as instructions. Per-IP token bucket
  via the shared `aiAllowed` bucket. Refused on the gate weeks.
